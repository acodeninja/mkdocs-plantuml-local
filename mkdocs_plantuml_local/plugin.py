...
class MkDocsPluginPlantUMLLocal(mkdocs.plugins.BasePlugin[MkDocsPluginPlantUMLLocalConfig]):
    ...
    def on_post_page(self, output: str, *, page: Page, config: MkDocsConfig) -> str | None:
        ...
        for index, block in enumerate(html.cssselect('pre.{}'.format(self.config.shortname))):
            start_time = time.time() * 1000

            plantuml_code = ''.join(block.cssselect('code')[0].itertext())
            plantuml_code = self._pre_render(plantuml_code)

            cache_keys = [json.dumps(list(self.config.values())), page.file.src_path, plantuml_code]

            svg = get_cache(*cache_keys)

            if not svg:
                self.logger.info(f'Rendering diagram {index + 1} of page {page.file.src_path}')
                svg = render(plantuml_code)
                put_cache(svg, *cache_keys)
            else:
                self.logger.info(f'Using cache for diagram {index + 1} of page {page.file.src_path}')
            ...
            self.logger.info(f'Handled diagram {index + 1} of page {page.file.src_path} in {end_time - start_time}ms')
