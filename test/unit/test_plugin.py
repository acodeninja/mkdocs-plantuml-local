...
@patch(
    "mkdocs_plantuml_local.plugin.check_dependencies",
...
def test_on_post_page_without_cache(
    put_cache,
    get_cache,
    time,
    render,
    ...
    mock_logger.info.assert_has_calls(
        [
            call("Rendering diagram 1 of page file"),
            call("Handled diagram 1 of page file in 0.0ms"),
        ]
    )

    get_cache.assert_called_once_with(
        json.dumps(list(plugin.config.values())), "file", ANY
    )
    put_cache.assert_called_once_with(
        ANY, json.dumps(list(plugin.config.values())), "file", ANY
    )
...
def test_on_post_page_without_cache(
    ...
    mock_logger.info.assert_has_calls(
        [
            call("Using cache for diagram 1 of page file"),
            call("Handled diagram 1 of page file in 0.0ms"),
        ]
    )
